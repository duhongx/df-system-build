  ...
  <div class="sql-page">
    ...
    <main class="right-panel">
      ...
      <div class="editor-section">
        ...
        <el-input v-model="form.content" type="textarea" :rows="16" placeholder="粘贴 SQL 内容，或从左侧选择文件查看" class="sql-textarea" />
        <div class="editor-actions">
          <el-button :loading="parsing" @click="handleParse">仅解析</el-button>
          <el-button type="warning" :loading="executing" @click="handleExecuteContent">解析并执行</el-button>
          <el-button type="primary" :disabled="!currentFile" :loading="executing" @click="handleExecuteFile()">执行选中文件</el-button>
          <el-button v-if="currentFile?.executeStatus === 'RUNNING'" type="danger" @click="handleCancelExecution">取消</el-button>
          <el-button v-if="currentBatch?.executeStatus === 'RUNNING'" type="danger" @click="handleCancelBatch">取消批次</el-button>
          <div class="toolbar-spacer" />
          <el-button link type="primary" :disabled="!currentFile" @click="handleExport">导出不可执行 SQL</el-button>
        </div>
      </div>
      ...
      <div class="result-section">
        <el-table :data="statements" size="small" border max-height="480" stripe>
        ...
        <el-empty v-if="!statements.length" :image-size="60" description="选择左侧文件或粘贴 SQL 后解析" />
      </div>
    </main>
  </div>
</template>
...
/* Left Panel */
.left-panel { width: 300px; flex-shrink: 0; background: #fff; border: 1px solid #ebeef5; border-radius: 6px; display: flex; flex-direction: column; overflow: hidden; }
...
/* Right Panel */
.right-panel { flex: 1; display: flex; flex-direction: column; gap: 12px; overflow: hidden; margin-left: 12px; }
.editor-section { background: #fff; border: 1px solid #ebeef5; border-radius: 6px; padding: 12px; }
...
/* Summary */
.summary-bar { display: flex; align-items: center; gap: 16px; padding: 8px 14px; background: #fff; border: 1px solid #ebeef5; border-radius: 6px; font-size: 13px; color: #606266; }
...
/* Batch files bar */
.batch-files-bar { display: flex; align-items: center; gap: 6px; flex-wrap: wrap; padding: 8px 14px; background: #fdf6ec; border: 1px solid #faecd8; border-radius: 6px; }
.batch-label { font-size: 13px; font-weight: 500; color: #e6a23c; margin-right: 8px; }
.batch-file-tag { cursor: pointer; }

/* Result */
.result-section { flex: 1; overflow: hidden; background: #fff; border: 1px solid #ebeef5; border-radius: 6px; padding: 12px; display: flex; flex-direction: column; }

@media (max-width: 1200px) {
  .sql-page { flex-direction: column; height: auto; }
  .left-panel { width: 100%; height: 300px; }
  .right-panel { margin-left: 0; margin-top: 12px; }
}
</style>
